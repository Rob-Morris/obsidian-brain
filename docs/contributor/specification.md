...
- `src/scripts/vendor_shaping_skill.py` — materialises the exact pinned portable shaping workflow from a verified clean Git checkout, refuses unmapped upstream membership, removes only stale provenance-owned vendor files, and retains Brain-owned composition files
