...
- **CLI wrapper boundary** — `build_index.py` and `search_index.py` remain supported script entrypoints, while internal runtime code now uses `_search.index`, `_search.assets`, and `_search.query` directly. The wrappers remain part of the public CLI surface, not the live internal runtime dependency graph.
