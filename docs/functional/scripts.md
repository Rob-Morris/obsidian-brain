...
- **CLI wrapper boundary** — `build_index.py` and `search_index.py` remain supported script entrypoints, while all internal Python code now uses `_search.index`, `_search.assets`, and `_search.query` directly. The wrappers are part of the public CLI surface only, not the internal Python import graph.
