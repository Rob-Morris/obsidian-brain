...
- **CLI wrapper boundary** — `build_index.py` and `search_index.py` remain supported script entrypoints, and converged internal code should prefer `_search.index`, `_search.assets`, and `_search.query` directly. Some MCP and lifecycle paths still route through the wrappers during the staged caller-convergence work, so the wrappers remain part of the current internal runtime surface as well as the public CLI surface.
