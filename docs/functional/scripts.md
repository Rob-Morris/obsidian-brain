...
- **CLI wrapper boundary** — `build_index.py` and `search_index.py` remain supported script entrypoints, while all internal Python code now uses canonical `_search` owners directly: `_search.index`, `_search.assets`, `_search.lexical_query`, `_search.semantic_query`, `_search.hybrid_query`, `_search.resource`, `_search.mode`, `_search.filters`, and `_search.snippet`. The wrappers are part of the public CLI surface only, not the internal Python import graph.
