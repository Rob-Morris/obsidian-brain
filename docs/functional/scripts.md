...
| `command.py` | Staged canonical selected-Brain noun/verb projection over `_application`; accepts one strict JSON request object, preserves canonical results and exit categories, and remains undispatched by current public CLI/MCP surfaces until coordinated cutover | `python3 command.py NOUN VERB [--request-json JSON\|-] [--vault V] [--workspace W] [--dry-run] [--json]` |
