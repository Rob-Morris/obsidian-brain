...
- Unix-like user install: `~/.local/bin/brain` and `~/.local/lib/brain-cli/4.0.4/`.
- Native Windows user install: `%LOCALAPPDATA%\Programs\Brain\bin\brain.cmd` and the adjacent `lib\brain-cli\4.0.4\` distribution.
...
The bootloader requires Python 3.12 or newer. `BRAIN_CLI_VERSION` is `4.0.4`; `BRAIN_INSTALL_REF` is `v0.70.5`.
