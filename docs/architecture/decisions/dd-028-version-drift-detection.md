...
The proxy also detects its own code drift via file-hash comparison (SHA-256) after child restarts, and injects upgrade notes into responses when drift is detected. The reader thread uses `select()` with a configurable timeout (default 30s) to detect children that hang without exiting — after 3 consecutive timeouts with in-flight requests, the proxy kills the child and routes that loss through the same restart coordinator. If every restart attempt fails, the proxy marks the session as given up and returns explicit `/mcp` restart guidance rather than a permanent soft-restarting loop. After give-up, later `tools/call` requests trigger an asynchronous VERSION re-check on the recovery thread; the triggering request still gets the unrecoverable error immediately, and recovery resumes only if `.brain-core/VERSION` changed. If the recovery thread itself crashes, dead-child requests surface a hard `MCP unrecoverable — proxy recovery thread crashed. Restart MCP via /mcp.` error instead of waiting forever.
