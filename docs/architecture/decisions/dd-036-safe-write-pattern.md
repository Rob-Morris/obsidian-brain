...
2. **Write to sibling temp file** — Content is written to `{target}.{pid}.tmp` in the same directory. Using a sibling (same filesystem, same directory) ensures `os.replace()` is atomic on POSIX — it is a single `rename(2)` syscall.
...
5. **Cleanup on any exception** — A `BaseException` handler unlinks the tmp file if any step fails, preventing `.pid.tmp` orphans.

The PID in the temp filename (`{target}.{pid}.tmp`) avoids collisions between concurrent processes writing to the same target.
...
- The tmp file is in the same directory, so it is visible in Obsidian briefly. The `.pid.tmp` suffix prevents Obsidian from treating it as a markdown file.
