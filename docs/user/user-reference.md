...
Use direct script invocation from a compatible Python 3.12+ launcher as the baseline command-line path. Bootstrap entrypoints such as `repair.py`, `setup.py`, `configure.py`, and the legacy `init.py` engine do their launcher-safe setup there; runtime-owning lifecycle entrypoints such as `repair.py`, `setup.py`, `configure.py`, `session.py`, and `check.py` then hand substantive managed work into the canonical managed runtime automatically.
...
| `vault_registry.py` | User-home authoritative Brain registry for local Brain IDs (currently typed `local` entries pointing at vault roots) |
| `workspace_registry.py` | Workspace key→path resolution and registration |
| `init.py` | Legacy/internal MCP registration engine beneath `configure.py mcp` and installer flows. It still owns the low-level Claude/Codex config writes and recorded removal path, and `--skip-mcp` remains the internal bootstrap-only path used by restricted installer flows. |
