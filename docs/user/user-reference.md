...
| `search_index.py` | Search the local retrieval index from the command line via lexical, semantic, or hybrid modes; hybrid preserves obvious exact-anchor lexical wins, gives a small tie-break boost to a clearly dominant semantic top result, and can apply a stronger semantic rescue when lexical and semantic leaders are clearly disjoint |
| `construct_benchmark_fixture.py` | Mine a real vault for lexical / semantic / hybrid / cluster / filter-sensitive benchmark cases and emit both a benchmark fixture JSON and an audit JSON; records bucket shortfall instead of padding weak cases, includes semantic-variant audit diagnostics such as best-variant and near-pure markers, can audit externally seeded semantic or hybrid candidates alongside locally mined ones, rejects seeded benchmark/fixture targets, and is intended to write vault-specific outputs under `.brain/local/benchmarks/` |
| `evaluate_search.py` | Benchmark lexical, semantic, and hybrid retrieval against a JSON query set so you can measure search quality before tuning it; reports per-bucket winner expectations as well as hit metrics, and semantic/hybrid modes require the optional semantic runtime |
