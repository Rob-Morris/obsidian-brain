...
- **brain_edit** — edit, append, prepend, or delete_section on an existing artefact or _Config/ resource (by path/basename for artefacts, by name for skill/memory/style/template); optional `target` parameter for section-level operations (required for delete_section); frontmatter merge strategy follows the operation verb (edit overwrites, append/prepend extend lists, null deletes field); artefacts auto-move to `+Status/` folders on terminal status change and back out on revive; config resources skip auto-move and modified injection
