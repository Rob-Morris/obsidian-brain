...
- Cross-type child folders use the owner's `{scope}/`
...
When a living artefact accumulates enough related sub-artefacts that the type folder gets crowded, the sub-artefacts move into a child subfolder determined by canonical ownership. The owner stays in the type root as the entry point; the subfolder groups its children.
