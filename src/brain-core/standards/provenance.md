...
**Terminal status:** If the source artefact has transferred all authority to the new one, set the terminal status in frontmatter and archive if you want it removed from the active namespace (see the [archiving standard](archiving.md)). Otherwise the callout alone suffices — the source remains active.
