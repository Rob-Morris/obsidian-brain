...
Reconciliation is idempotent: a second pass is a no-op. It runs only on write paths (`document.edit` for an artefact, `artefact.rename`, migration) — `artefact.read` is side-effect-free. Once reconciled, the values are written back to frontmatter and the filename is re-rendered from the selected rule; any disagreement is resolved in favour of the reconciled frontmatter.
