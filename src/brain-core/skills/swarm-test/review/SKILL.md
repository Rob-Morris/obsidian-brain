...
Create 8-12 test scenarios (adjust to the target's scope). Each scenario is a **task a real person would try to accomplish** using the work product. Design for coverage:
...
Launch all agents in parallel using the smallest available model (in Claude Code: `model: haiku`). Each agent gets:
...
Present proposed fixes to the user for approval before implementing. Then apply the same judgment you would for any change:
