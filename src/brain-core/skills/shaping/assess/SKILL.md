...
6. Call `shaping.start(target="{path}", mode="{mode}")`. This validates shapeability, transitions lifecycle status through the canonical handler, and creates or continues today's transcript.
...
- **Research spikes.** If the user seems uncertain about a question, offer a spike: "Sounds like you're not sure — want me to research this and propose some options?" If they agree, do it inline — create a child research artefact via `artefact.create(type="research", ...)`, investigate, fold findings back into the parent artefact, and resume shaping. The spike is an offer, not a claim — the user might just need to think.
