...
2. `brain_list(resource="trigger")` — workflow triggers to follow throughout the session
...
- `brain_read(resource=..., name=...)` for reading specific config items (types, triggers, styles, templates, skills, plugins, memories)
- `brain_list(resource=...)` for listing collections (skills, triggers, styles, etc.)
