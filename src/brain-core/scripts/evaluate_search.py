...
evaluation tool for tuning inline semantic-search behaviour without changing
the underlying search contract.
...
def _build_parser():
    """Build the evaluate_search CLI parser."""
    parser = RaisingArgumentParser(prog="evaluate_search.py")
    parser.add_argument("--benchmark", help="Path to the benchmark fixture JSON")
    parser.add_argument("--vault", help="Override the detected vault root")
    parser.add_argument(
        "--mode",
        action="append",
        default=[],
        help="Restrict evaluation to one retrieval mode; may be provided multiple times",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit the scorecard as JSON instead of the formatted report",
    )
    return parser
...
def parse_args(argv):
    ...
    require_option(parsed, "benchmark", "--benchmark")
    return parsed.benchmark, parsed.vault, parsed.mode, parsed.json
