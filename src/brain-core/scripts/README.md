...
| `search_index.py` | Thin CLI/script wrapper over `_search`: lexical, semantic, or hybrid local search with exact-anchor lexical wins, strong semantic champions, Brain-only title champions, and semantic-rescue fusion for disjoint leaders. Use `_search.query` directly from Python; the wrapper remains only as a supported script entry surface. | `python3 search_index.py "query" [--type T] [--mode M] [--json]` |
