...
| `doctor_machine.py` | Launcher-safe machine-level runtime diagnosis adapter used by `brain doctor` when a source Brain is available; reports shared-runtime topology from the `_machine/` substrate while the CLI keeps the shell/bootstrap fallback | (internal helper, called by `brain doctor`) |
