...
def _build_parser():
    """Build the construct_benchmark_fixture CLI parser."""
    parser = RaisingArgumentParser(prog="construct_benchmark_fixture.py")
    parser.add_argument(
        "--fixture-out",
        help="Path to write the benchmark fixture JSON",
    )
    parser.add_argument("--audit-out", help="Path to write the audit JSON")
    parser.add_argument("--vault", help="Override the detected vault root")
    parser.add_argument(
        "--target-lexical",
        type=int,
        help="Target case count for lexical-expected examples",
    )
    parser.add_argument(
        "--target-semantic",
        type=int,
        help="Target case count for semantic-expected examples",
    )
    parser.add_argument(
        "--target-hybrid",
        type=int,
        help="Target case count for hybrid-expected examples",
    )
    parser.add_argument(
        "--target-cluster",
        type=int,
        help="Target case count for cluster-expected examples",
    )
    parser.add_argument(
        "--target-filter",
        type=int,
        help="Target case count for filter-sensitive examples",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit the result payload as JSON instead of the default stderr summary",
    )
    parser.add_argument(
        "--semantic-strategy",
        choices=(
        ...
        default=SEMANTIC_STRATEGY_LOCAL,
        help="Semantic mining strategy to use for benchmark generation",
    )
    parser.add_argument(
        "--semantic-seed-file",
        help="Optional JSON file providing additional semantic seed queries",
    )
    parser.add_argument(
        "--hybrid-seed-file",
        help="Optional JSON file providing additional hybrid seed queries",
    )
    return parser
...
def parse_args(argv):
    ...
    require_option(parsed, "fixture_out", "--fixture-out")
    targets = dict(DEFAULT_TARGETS)
    if parsed.target_lexical is not None:
        targets["lexical-expected"] = parsed.target_lexical
    if parsed.target_semantic is not None:
        targets["semantic-expected"] = parsed.target_semantic
    if parsed.target_hybrid is not None:
        targets["hybrid-expected"] = parsed.target_hybrid
    if parsed.target_cluster is not None:
        targets["cluster-expected"] = parsed.target_cluster
    if parsed.target_filter is not None:
        targets["filter-sensitive"] = parsed.target_filter
    ...
    return (
        ...
        targets,
        parsed.json,
        parsed.semantic_strategy,
        parsed.semantic_seed_file,
        parsed.hybrid_seed_file,
    )
