...
class ArtefactEditRequest:
    COMMAND_ID: ClassVar[str] = "artefact.edit"
...
def execute(context: InvocationContext, request: ArtefactEditRequest):
    return execute_document_edit(
        context, request, resource="artefact", operation="edit", subject_field="path"
    )


def decode(payload: Mapping[str, object]) -> ArtefactEditRequest:
    return decode_structural_request(
        payload, ArtefactEditRequest, subject_field="path", allow_fix_links=True
    )


def catalogue_entry():
    return contributor_mutation_entry(ArtefactEditRequest, execute)


def resolver_entry():
    from ..resolver import ResolverEntry

    return ResolverEntry(ArtefactEditRequest, decode)
