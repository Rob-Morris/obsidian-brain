...
4. **Generate PDF.** When the user is happy, the tool can also be used to render the final PDF, or run manually: `marp {path} --theme {theme_path} -o _Attachments/yyyymmdd-deck--{slug}.pdf`
...
3. Generate PDF: `marp {path} --theme _Config/Skills/presentations/theme.css -o _Attachments/yyyymmdd-deck--{slug}.pdf`
