...
[4] **Docs updated.** Update every file affected by your change:
    ...
    [4b] **Template vault defaults** — if added/removed: `src/brain-core/guide.md` (type table), `docs/specification.md` (starter vault section)
    ...
    [4f] **System design/architecture** — `docs/architecture/overview.md`, `docs/architecture/decisions/` (if new design decision), `src/brain-core/index.md` (if principles change)
    [4g] **Day-to-day workflows** — `docs/user/workflows.md`, `src/brain-core/guide.md`
    [4h] **Tooling** — scripts, MCP tools, config: `docs/functional/scripts.md` (scripts), `docs/functional/mcp-tools.md` (MCP tools), `docs/functional/config.md` (config), `src/brain-core/scripts/README.md` (co-located module map)
...
[5] **Shared facts cross-checked.** Grep for the specific values you changed to catch stale references in other files. With the three-layer structure, most facts now live in one place — only check if you changed a shared fact:
    ...
    [5b] **Template vault defaults list** — `src/brain-core/guide.md`, `docs/specification.md`
    [5c] **Status values** — `src/brain-core/artefact-library/README.md`, `docs/user/system-guide.md`
    [5d] **Install step counts** — `docs/user-reference.md`, `src/brain-core/artefact-library/README.md`, `src/brain-core/standards/extending/`
