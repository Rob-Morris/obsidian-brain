...
class TestParseArgs:
    ...
    def test_requires_fixture_out(self):
        with pytest.raises(ValueError, match="the following arguments are required: --fixture-out"):
            cbf.parse_args(["construct_benchmark_fixture.py"])

    def test_rejects_unknown_flag(self):
        with pytest.raises(ValueError, match="unrecognized arguments: --bogus"):
            cbf.parse_args(["construct_benchmark_fixture.py", "--bogus"])

    def test_rejects_missing_target_value(self):
        with pytest.raises(ValueError, match="argument --target-lexical: expected one argument"):
            cbf.parse_args(
                [
                    "construct_benchmark_fixture.py",
                    "--fixture-out",
    ...
    def test_main_exits_on_unknown_flag(self, monkeypatch, capsys):
        monkeypatch.setattr(cbf.sys, "argv", ["construct_benchmark_fixture.py", "--bogus"])
