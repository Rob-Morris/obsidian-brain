...
from brain_test_support import make_searchable_vault
...
@pytest.fixture
def vault(tmp_path):
    """Create a vault with searchable content (shared corpus, see brain_test_support)."""
...
class TestBuildReport:
    ...
    def test_parse_args_requires_benchmark(self):
        with pytest.raises(ValueError, match="the following arguments are required: --benchmark"):
            es.parse_args(["evaluate_search.py"])

    def test_parse_args_rejects_unknown_flag(self):
        with pytest.raises(ValueError, match="unrecognized arguments: --bogus"):
            es.parse_args(["evaluate_search.py", "--bogus"])

    def test_parse_args_rejects_missing_mode_value(self):
        with pytest.raises(ValueError, match="argument --mode: expected one argument"):
            es.parse_args(["evaluate_search.py", "--benchmark", "bench.json", "--mode"])

    def test_main_exits_on_unknown_flag(self, monkeypatch, capsys):
        monkeypatch.setattr(es.sys, "argv", ["evaluate_search.py", "--bogus"])
