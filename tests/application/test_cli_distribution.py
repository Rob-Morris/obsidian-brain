...
def test_installed_pair_is_versioned_manifested_and_runnable(tmp_path):
    ...
    assert installed.distribution_root == tmp_path / "prefix" / "lib" / "brain-cli" / "2.0.0"
    assert manifest["cli_version"] == "2.0.0"
...
def test_windows_target_selects_the_cmd_bootloader(tmp_path):
    ...
    assert verify_distribution(installed.distribution_root)["cli_version"] == "2.0.0"
...
def test_unverified_old_distribution_is_reported_honestly_on_failure(tmp_path):
    ...
    distribution = tmp_path / "prefix" / "lib" / "brain-cli" / "2.0.0"
